...
import (
	...
	"github.com/jgraettinger/cockroach-encoding/uint128"
)

var errResultOutOfRange = errors.New("numeric value out of range")
...
func (ipAddr *IPAddr) FromBuffer(data []byte) ([]byte, error) {
	...
	if ipAddr.Family != IPv4family && ipAddr.Family != IPv6family {
		return nil, fmt.Errorf("IPAddr decoding error: bad family, got %d", ipAddr.Family)
	}
	ipAddr.Mask = data[1]
...
func ParseINet(s string, dest *IPAddr) error {
	...
	if i < 0 {
		...
		ip := net.ParseIP(addr)
		if ip == nil {
			return fmt.Errorf("could not parse %q as inet. invalid IP", s)
		}
	...
	if err != nil {
		return fmt.Errorf("could not parse %q as inet. invalid mask", s)
	...
	if family == IPv4family {
		...
		if (octetCount+1)*8-1 < maskOnes {
			return fmt.Errorf("could not parse %q as inet. mask is larger than provided octets", s)
		}
...
func (ipAddr *IPAddr) And(other *IPAddr) (IPAddr, error) {
	var newIPAddr IPAddr
	if ipAddr.Family != other.Family {
		return newIPAddr, fmt.Errorf("cannot AND inet values of different sizes")
	}
	newIPAddr.Family = ipAddr.Family
...
func (ipAddr *IPAddr) Or(other *IPAddr) (IPAddr, error) {
	var newIPAddr IPAddr
	if ipAddr.Family != other.Family {
		return newIPAddr, fmt.Errorf("cannot OR inet values of different sizes")
	}
	newIPAddr.Family = ipAddr.Family
...
func (ipAddr *IPAddr) SubIPAddr(other *IPAddr) (int64, error) {
	var diff int64
	if ipAddr.Family != other.Family {
		return diff, fmt.Errorf("cannot subtract inet addresses with different sizes")
	}
